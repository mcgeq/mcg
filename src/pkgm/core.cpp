...
namespace mg::pkgm
{
auto plan_command(std::string_view action,
                  const CommandArgs& command_args,
                  const PackageOptions& options)
    -> std::expected<PlannedCommand, MgError>
{
  return plan_command_from_path(std::filesystem::current_path(),
                                action,
                                command_args,
                                options);
}

auto plan_command_from_path(const std::filesystem::path& start_dir,
                            std::string_view action,
                            const CommandArgs& command_args,
                            const PackageOptions& options)
    -> std::expected<PlannedCommand, MgError>
{
  const auto manager = detect_package_manager_for_command_from_path(start_dir,
                                                                    action,
                                                                    command_args);
  if (!manager) {
...
auto execute_command(std::string_view action,
                     const CommandArgs& command_args,
                     const PackageOptions& options) -> std::expected<void, MgError>
{
  ...
  auto planned = plan_command_from_path(start_dir, action, command_args, options);
  if (!planned) {
