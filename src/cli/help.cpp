...
constexpr auto k_fs_help_lines = std::array {
    ...
    std::string_view {"Options before subcommand: --cwd/-C <path>, --dry-run/-d"},
    std::string_view {
        "Subcommands: create(c,touch), remove(r), copy(y), move(m), list(ls), exists(test), read(cat), write(echo)"},
};
}  // namespace

auto main_help_lines() noexcept -> std::span<const std::string_view>
{
  return k_main_help_lines;
}

auto fs_help_lines() noexcept -> std::span<const std::string_view>
{
  return k_fs_help_lines;
}

auto version_line() -> std::string_view
{
  static const auto line = std::string {"mg "} + std::string {project_version()};
  return line;
}
